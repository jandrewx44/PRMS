        ...
        <div class="row mb-2">
          <div class="col-12">
            <div class="btn-group btn-group-sm" role="group" aria-label="Record filters">
              ...
              <a href="confirmation.php?home=confirmation" class="btn btn-primary text-white">Confirmation</a>
              <a href="marriage.php?home=marriage" class="btn btn-outline-light text-dark bg-white">Marriage</a>
              <a href="communion.php?home=communion" class="btn btn-outline-light text-dark bg-white">Communion</a>
      ...
      <div class="container-fluid">
        <div class="row">
		  ...
		  <?php
            ...
            <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#search_confirmation_name">
              ...
              <div class="row">
				...
				$currenturl="http://".$_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
				 if (isset($_GET['page_no']) && $_GET['page_no']!="") {
					...
					$result_count = mysqli_query($conn,"SELECT COUNT(DISTINCT DATE_FORMAT(DATE_CONFIRMED, '%Y')) As total_records FROM tbl_confirmation");
					$total_records = mysqli_fetch_array($result_count);
					$total_records = $total_records['total_records'];
					$total_no_of_pages = ceil($total_records / $total_records_per_page);
					$second_last = $total_no_of_pages - 1; // total page minus 1
		
					$news = "SELECT COUNT(DATE_CONFIRMED) As tTotal, DATE_FORMAT(DATE_CONFIRMED, '%Y') As tDate FROM tbl_confirmation GROUP BY DATE_FORMAT(DATE_CONFIRMED, '%Y') ORDER BY DATE_FORMAT(DATE_CONFIRMED, '%Y') DESC LIMIT $offset, $total_records_per_page"; 
				...
				?>
						...
						<div class="small-box bg-info">
						  ...
						  <a data-mytooltip="tooltip" data-placement="top" title="VIEW RECORD" href="confirmationn_View.php?year=<?=$group;?>" class="small-box-footer">More info <i class="fas fa-arrow-circle-right"></i></a>
						</div>
					  </div>
